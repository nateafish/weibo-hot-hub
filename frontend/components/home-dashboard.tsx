  ...
  return <div className="page"><PageHeader title="今日微博热搜" subtitle="按小时保存榜单，并将热度趋势、正文与智搜版本关联到可检索的话题档案。" aside={status} />
    {error ? <ErrorState message={error} /> : !day ? <div className="loading-block h-96" /> : <>
      <Tabs defaultValue="latest">
        ...
        <TabsContent value="latest"><div className="flex flex-wrap gap-2">{day.hours.map((item) => <Button size="sm" variant={item.hour === current?.hour ? "default" : "outline"} onClick={() => setHour(item.hour)} key={item.hour}>{item.hour}:00</Button>)}</div><HotlistTable entries={latestEntries} onSelect={setSelected} /></TabsContent>
